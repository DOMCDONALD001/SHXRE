...
<p align="center">
   The most SHXRE-like experience that is open source!
</p>
...
-- Users can add images, GIFs and videos to posts
...
1. Clone the repository
   ...
   ```bash
   git clone https://github.com/your-repo/SHXRE
   ```
...
If you have a Firebase service account JSON and want to wire server secrets into `.env.development`, use the included helper script:

```bash
# from repo root
./scripts/setup-firebase-env.sh /path/to/service-account.json
```

The script requires `jq` and `python3` and will back up your `.env.development` before updating `FIREBASE_PRIVATE_KEY`, `FIREBASE_CLIENT_EMAIL`, `FIREBASE_PROJECT_ID`, and `FIREBASE_DATABASE_URL`.
