...
import { trendsCollection, postsCollection } from '@lib/firebase/collections';
import {
  manageReply,
  uploadImages,
  manageTotalTweets,
...
export function Input({
  ...
  const sendTweet = async (): Promise<void> => {
    try {
      ...
      let images: ImagesPreview | null = null;

      try {
        images = await uploadImages(userId, selectedImages);
      } catch (err) {
        console.error('uploadImages error', err);
        toast.error('Image upload failed — try smaller files or different images');
        setLoading(false);
        return;
      }
      ...
      const [tweetRef] = await Promise.all([
        addDoc(postsCollection, tweetData),
